...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Live index/candle/options evidence | Angel readers and normalizers | PARTIAL | `certified_live_provider_readers.py`, `angel_live_observation_normalizer.py` | provider-contract proof and unavailable handling | none | source/freshness evidence | 1A source certification |
| 1 | India VIX | diagnostic-only capture | BLOCKED | `india_vix_provider_contract_capture.py`, `VolatilitySnapshotV1` | authoritative timestamp/prior-close semantics | provider evidence | accepted diagnostic package | 1B VIX certification |
| 2 | Fourteen-pillar evidence | typed P5 engines and candidates | PARTIAL | `technical_intelligence`, `option_chain_intelligence`, `market_regime`, `broader_market_intelligence` | active-composition completeness | 1 | pillar matrix and fail-closed tests | 2A evidence closure |
| 2 | Two-market selection | parent coordinator/canary | PARTIAL | `two_market_parent_cycle_coordinator.py`, Task 8 contracts | complete live evidence wiring | 1, 2A | exact-once winner/loser evidence | 2B parent certification |
