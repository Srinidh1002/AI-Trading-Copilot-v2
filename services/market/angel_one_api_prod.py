# services/market/angel_one_api_prod.py - PRODUCTION ANGEL ONE API
...
class AngelOneAPIProd:
    """Production Angel One API with correct endpoints."""
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.info(f"[ANGEL] Status: {response.status_code}")
            logger.info(f"[ANGEL] Response: {response.text[:200]}")
            
            if response.status_code == 200:
                try:
                    result = response.json()
                except:
                    logger.error("[ANGEL] Invalid JSON response")
                    return False
                ...
                if result.get('status') == True or result.get('status') == 'true':
                    ...
                    logger.error(f"[ANGEL] Login failed: {result.get('message', 'Unknown')}")
                    return False
            else:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if data.get('status') == True or data.get('status') == 'true':
                        ...
                        logger.error(f"[ANGEL] API error: {data.get('message', 'Unknown')}")
                        return {}
                except:
                    logger.error(f"[ANGEL] Invalid JSON response from {endpoint}")
                    return {}
            else:
                logger.error(f"[ANGEL] HTTP {response.status_code} from {endpoint}")
                return {}
                
        except Exception as e:
            logger.error(f"[ANGEL] Request error for {endpoint}: {e}")
            return {}
    ...
    def get_ltp(self, symbol: str) -> float:
        ...
        data = self._make_request(
            "POST", 
            "/rest/secure/angelbroking/quote/v1/ltp",
    ...
    def get_option_chain(self, symbol: str, expiry: str = None) -> dict:
        ...
        data = self._make_request(
            "POST",
            "/rest/secure/angelbroking/option/v1/getOptionChain",
            {"symbol": symbol, "expiry": expiry}
        )
        
        if data:
            result = self._parse_option_chain(data, symbol)
