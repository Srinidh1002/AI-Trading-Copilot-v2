...
def build_canonical_multi_timeframe_snapshot(*,candle_series_by_timeframe,policy=DEFAULT_MULTI_TIMEFRAME_POLICY,freshness_policy=DEFAULT_MARKET_DATA_FRESHNESS_POLICY,clock=None,timeframe_evidence_id_factory=None,quality_result_id_factory=None,snapshot_id_factory=None,multi_timeframe_quality_result_id_factory=None):
 ...
 ordered=tuple(sorted(series,key=lambda s:policy.required_timeframes.index(s.timeframe)));evidence=tuple(build_timeframe_evidence(s,policy=policy,freshness_policy=freshness_policy,clock=lambda:now,quality_result_id_factory=quality_result_id_factory,timeframe_evidence_id_factory=timeframe_evidence_id_factory) for s in ordered);symbol,exchange=ordered[0].underlying_symbol,ordered[0].exchange;missing=set(policy.required_timeframes)-{s.timeframe for s in ordered};optional=tuple(t for t in policy.optional_timeframes if t in missing);snapshot=MultiTimeframeSnapshotV1((snapshot_id_factory or (lambda:"mtf-snapshot"))(),now,symbol,exchange,policy.required_timeframes,evidence,policy.anchor_timeframe,None,blockers=("missing_timeframes",) if set(policy.mandatory_timeframes)&missing else (),warnings=tuple(f"optional_timeframe_unavailable_{t}" for t in optional));return snapshot,evaluate_multi_timeframe_quality(snapshot,policy=policy,clock=lambda:now,quality_result_id_factory=multi_timeframe_quality_result_id_factory)
