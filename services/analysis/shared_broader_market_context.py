...
def build_certified_shared_broader_context(*, nifty_observation: AngelLiveMarketObservationV1, sensex_observation: AngelLiveMarketObservationV1, evaluated_at: datetime, india_vix_capture: IndiaVixCaptureResultV1|None=None) -> CertifiedSharedMarketContextV1:
    ...
    blockers: tuple[str, ...] = (); vix=normalize_india_vix_capture(india_vix_capture) if india_vix_capture is not None else (None,None)
    ...
    return CertifiedSharedMarketContextV1(
        cycle_id=f"shared-broader:{evaluated_at.isoformat()}", evaluated_at=evaluated_at,
        ...
        source_timestamps=timestamps, india_vix_capture=india_vix_capture, blockers=blockers,
        cache_metadata={"india_vix_normalization": {"nifty_snapshot_id": vix[0].volatility_snapshot_id if vix[0] else None, "sensex_snapshot_id": vix[1].volatility_snapshot_id if vix[1] else None}},
    )
