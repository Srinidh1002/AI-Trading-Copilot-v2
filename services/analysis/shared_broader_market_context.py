...
from services.contracts.broader_market_intelligence_policy_v1 import DEFAULT_BROADER_MARKET_INTELLIGENCE_POLICY
...
_TASK9_BROADER_MARKET_POLICY = replace(
    DEFAULT_BROADER_MARKET_INTELLIGENCE_POLICY,
    policy_name="TASK9_REQUIRED_INDIA_VIX_BROADER_MARKET_POLICY",
    require_volatility_context=True,
    warn_on_missing_optional_evidence=False,
)


def build_certified_shared_broader_context(*, nifty_observation: AngelLiveMarketObservationV1, sensex_observation: AngelLiveMarketObservationV1, evaluated_at: datetime, cross_market_evaluated_at: datetime | None = None, india_vix_capture: IndiaVixCaptureResultV1|None=None, capture_diagnostics: dict | None = None, shared_external_context: SharedExternalMarketContextV1 | None = None) -> CertifiedSharedMarketContextV1:
    """Build both primary canonical results from already-normalized 5m candles."""
    evaluated_at = _aware(evaluated_at)
    cross_market_evaluated_at = (
        evaluated_at
        if cross_market_evaluated_at is None
        else _aware(cross_market_evaluated_at)
    )
    if cross_market_evaluated_at > evaluated_at:
        raise ValueError("cross market evaluation boundary")
    ...
    if CORRELATION_TIMEFRAME not in nifty or CORRELATION_TIMEFRAME not in sensex:
        ...
        nifty_result = build_broader_market_intelligence(policy=_TASK9_BROADER_MARKET_POLICY,
            ...
            created_at=evaluated_at, cross_market_created_at=cross_market_evaluated_at, cross_market_evidence_id=f"{prefix}:nifty-cross", result_id=f"{prefix}:nifty",
        )
        sensex_result = build_broader_market_intelligence(policy=_TASK9_BROADER_MARKET_POLICY,
            ...
            created_at=evaluated_at, cross_market_created_at=cross_market_evaluated_at, cross_market_evidence_id=f"{prefix}:sensex-cross", result_id=f"{prefix}:sensex",
        )
