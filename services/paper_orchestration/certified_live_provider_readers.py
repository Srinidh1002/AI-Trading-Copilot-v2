...
class CertifiedIndexMarketSpecV1:
    ...
    def __post_init__(self) -> None:
        ...
        token = str(self.symboltoken).strip()
        option_exchange = str(self.option_exchange).strip().upper()

        allowed = {
            ("NIFTY", "NSE", "99926000", "NFO"),
            ("SENSEX", "BSE", "99919000", "BFO"),
        }
        if (symbol, exchange, token, option_exchange) not in allowed:
            raise ValueError("unsupported certified index market specification")

        object.__setattr__(self, "underlying_symbol", symbol)
        object.__setattr__(self, "exchange", exchange)
        object.__setattr__(self, "symboltoken", token)
...
def market_spec_for(
    ...
    raise ValueError("certified live readers support only NIFTY/NSE and SENSEX/BSE")
...
class CertifiedLiveProviderReaders:
    """Concrete read-only adapters around existing live analysis pipelines.

    Provider objects and quote acquisition are injected at the composition
    root. This module never acquires credentials and never submits orders.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(getattr(analysis_pipeline, "analyse", None)):
            raise TypeError("analysis_pipeline must expose analyse()")
        if not callable(getattr(option_decision_pipeline, "analyse", None)):
            raise TypeError("option_decision_pipeline must expose analyse()")

        self.quote_reader = quote_reader
        self.analysis_pipeline = analysis_pipeline
        self.option_decision_pipeline = option_decision_pipeline
        self.available_capital = _positive(
            available_capital,
            "available_capital",
        )
        self.risk_percent = _positive(risk_percent, "risk_percent")
    ...
    def read_data(
        ...
        spot_price = raw.get(
            "spot_price",
            raw.get("ltp", raw.get("last_price")),
        )
        market_timestamp = _aware(
            raw.get("market_timestamp", cycle_input.market_timestamp),
            "market_timestamp",
        )
        received_at = _aware(
            raw.get("received_at", cycle_input.received_at),
            "received_at",
        )
    ...
    def read_opportunity(
        ...
        if type(analysis_result) is not CertifiedLiveAnalysisResultV1:
            raise TypeError(
                "analysis_result must be exact CertifiedLiveAnalysisResultV1"
            )
        ...
        spot_price = analysis_result.analysis.get("spot_price")
        if spot_price is None:
            spot_price = analysis_result.analysis.get("ltp")
        if spot_price is None:
            raise ValueError(
                "analysis result must expose spot_price or ltp for option analysis"
            )
        ...
        raw = _mapping(
            self.option_decision_pipeline.analyse(
                ...
                session_now=cycle_input.market_timestamp,
            ),
        ...
        if decision in {"BUY", "TRADE", "TRADE_READY", "READY"}:
            status = "READY"
        elif decision in {"CONFLICTING", "MIXED"}:
            status = "CONFLICTING"
            blockers = blockers or ("CONFLICTING_SIGNALS",)
        elif decision in {"FAILED", "ERROR"}:
            status = "FAILED"
            blockers = blockers or ("UPSTREAM_FAILURE",)
        elif blockers:
            status = "BLOCKED"
        else:
            status = "NO_ACTION"
        ...
        return {
            ...
            "evidence": {
                ...
                "analysis_observation_id": analysis_result.observation_id,
                "execution_mode": "PAPER",
                "live_execution_eligible": False,
                "broker_order_submission": False,
            },
