...
def compose_task2_decision_certification(fixture:Task2TwoMarketFixtureV1)->Task2DecisionCertificationResultV1:
 ...
 def build(item):
  ...
  ledger=build_market_analysis_confidence_ledger(cycle_id=fixture.parent_cycle_id,observation_id=item.observation_id,evidence=canonical.evidence,policy_source=policy);calls["confidence_ledger_build_count"]+=1
  if candidate is None:
   action=resolve_pre_entry_market_action(candidate=None,cycle_id=fixture.parent_cycle_id,observation_id=item.observation_id,evaluated_at=fixture.fixed_evaluated_at,failure_codes=item.controlled_blockers or ("CANDIDATE_UNAVAILABLE",)); calls["pre_entry_action_resolution_count"]+=1
   ...
   explanation=build_market_decision_explanation(candidate=None,action=action,cycle_id=fixture.parent_cycle_id,observation_id=item.observation_id,evaluated_at=fixture.fixed_evaluated_at,contributions=contributions,ledger=ledger,terminal_codes=terminal.blockers); calls["child_explanation_build_count"]+=1
   return candidate,ledger,contributions,action,explanation,terminal
  action=resolve_pre_entry_market_action(candidate=candidate,cycle_id=fixture.parent_cycle_id,observation_id=item.observation_id,evaluated_at=fixture.fixed_evaluated_at,ledger=ledger); calls["pre_entry_action_resolution_count"]+=1
  ...
  explanation=build_market_decision_explanation(candidate=candidate,action=action,cycle_id=fixture.parent_cycle_id,observation_id=item.observation_id,evaluated_at=fixture.fixed_evaluated_at,contributions=contributions,ledger=ledger); calls["child_explanation_build_count"]+=1
  return candidate,ledger,contributions,action,explanation,terminal
 nifty=build(fixture.nifty);sensex=build(fixture.sensex)
 decision=rank_two_market_candidates(decision_result_id=f"decision:{fixture.scenario_id}",parent_cycle_id=fixture.parent_cycle_id,requested_at=min(nifty[5].requested_at,sensex[5].requested_at),completed_at=max(nifty[5].received_at,sensex[5].received_at),nifty=nifty[5],sensex=sensex[5],policy=TwoMarketDecisionPolicyV1(180.,5.));calls["parent_rank_evaluation_count"]+=1
 projection=project_parent_pre_entry_action(parent_decision=decision,nifty_action=nifty[3],sensex_action=sensex[3]);calls["parent_action_projection_count"]+=1
 parent_explanation=build_two_market_decision_explanation(parent_decision=decision,nifty_action=nifty[3],sensex_action=sensex[3],nifty_explanation=nifty[4],sensex_explanation=sensex[4],parent_action_projection=projection);calls["parent_explanation_build_count"]+=1
