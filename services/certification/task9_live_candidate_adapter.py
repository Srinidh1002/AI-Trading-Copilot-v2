"""Strict Task 9 bridge from captured live evidence to typed candidates.
...
_TASK9_EXTERNAL_CONTEXT_SUBSYSTEMS = (
...
def _task9_external_context_for_decision(
    external_context,
):
    """Project retained external evidence into Task 9 decision input.

    The shared context remains the durable audit authority.

    An UNAVAILABLE external context is omitted from decision evaluation
    only when every Task 9 subsystem represented by that context is
    explicitly non-blocking.

    READY / READY_WITH_WARNINGS evidence remains usable augmentation.
    BLOCKED / CONFLICTING evidence remains supplied and therefore
    continues to fail closed.

    If a future subsystem role becomes decision-blocking, an unavailable
    external context is supplied again automatically and fails closed.
    """
    ...
    registry = {
        state.subsystem_id: state
        for state
        in default_task9_subsystem_role_registry()
    }

    states = tuple(
        registry[subsystem_id]
        for subsystem_id
        in _TASK9_EXTERNAL_CONTEXT_SUBSYSTEMS
    )

    if any(
        state.decision_blocking
        for state in states
    ):
        return external_context
...
def _policy_source(
    ...
    *,
    ...
    # The default keeps the already-certified Task 9 adapter in its explicit
    ...
    return LiveCandidatePolicySourceV1.unavailable(
        blockers=("CANONICAL_POLICY_UNAVAILABLE",)
    )

def evaluate_task9_live_candidate(
    cycle_input: PaperOrchestrationCycleInputV1,
    data_result: CertifiedLiveDataResultV1,
    supplied_analysis: Mapping[str, object],
    ...
    *,
    ...
    """Return the complete provider-free Task 9 typed evaluation.
    ...
    try:
        ...
        if not hasattr(exc, "task9_failure_stage"):
            setattr(
                exc,
                "task9_failure_stage",
                "TASK9_SHARED_CONTEXT_PROJECTION",
            )
        raise

    explicit_policy_supplied = (
        supplied_analysis.get(
            "canonical_directional_policy"
        )
        is not None
        or all(
            supplied_analysis.get(name)
            is not None
            for name in (
                "policy_direction",
                "policy_eligibility",
    ...
    def canonical_policy_factory(
        evidence,
    ) -> LiveCandidatePolicySourceV1:
        # Preserve explicit already-certified policy inputs for bounded
        # compatibility/testing. The official Task 9 production path does
        # not supply these and therefore derives its policy below.
        if explicit_policy_supplied:
        ...
        canonical = (
            evaluate_canonical_directional_policy(
                policy_id=(
                    "task9-canonical:"
                    f"{parent_cycle_id}:"
                    f"{cycle_input.observation_id}"
                ),
                symbol=expected[0],
                exchange=expected[1],
                evaluated_at=(
                    evaluation_capture.evaluated_at
                ),
                evidence=family_set,
            )
        )

        # Reuse the existing strict conversion boundary. Legacy fallback
        # remains disabled.
        return _policy_source(
            {
                "canonical_directional_policy":
    ...
    try:
        result = (
            evaluate_captured_certified_market_candidate_with_policy_factory(
                ...
                session_validation=(
                    cycle_input.session_validation
                ),
                policy_factory=(
                    canonical_policy_factory
                ),
                parent_cycle_id=parent_cycle_id,
                candidate_id=(
                    f"certified-live:"
    ...
    try:
        if candidate_identity != expected_identity:
            raise ValueError(
                "evaluated candidate identity mismatch"
            )
        if candidate.market_timestamp != data_result.market_timestamp:
            raise ValueError(
                "evaluated candidate timestamp mismatch"
            )
    except Exception as exc:
        if not hasattr(exc, "task9_failure_stage"):
            setattr(
                exc,
                "task9_failure_stage",
                "TASK9_POST_EVALUATION_IDENTITY",
            )
        raise
...
def adapt_task9_live_candidate(
    cycle_input: PaperOrchestrationCycleInputV1,
    data_result: CertifiedLiveDataResultV1,
    supplied_analysis: Mapping[str, object],
    ...
    *,
    ...
    """CandidateReader-compatible Task 9 adapter.

    Captured live evidence uses ``evaluate_task9_live_candidate``. Exact
    ...
    if (
        ...
        return evaluate_task9_live_candidate(
            cycle_input,
            data_result,
            supplied_analysis,
            captured_evidence,
...
Task9EvaluationSink = Callable[
    [str, LiveMarketCandidateEvaluationResultV1],
    None,
]


def build_task9_retaining_candidate_reader(
    *,
    evaluation_sink: Task9EvaluationSink,
):
    ...
    def reader(
        ...
        *,
        ...
        if (
            ...
            evaluation = evaluate_task9_live_candidate(
                cycle_input,
                data_result,
                supplied_analysis,
                captured_evidence,
        ...
        return adapt_task9_live_candidate(
            cycle_input,
            data_result,
            supplied_analysis,
            captured_evidence,
