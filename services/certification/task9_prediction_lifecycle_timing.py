...
def resolve_prediction_lifecycle_window(*, prediction_record: PredictionRecordV1, session_policy: MarketSessionPolicy, timing_policy: PredictionLifecycleTimingPolicyV1 = PredictionLifecycleTimingPolicyV1()) -> PredictionLifecycleWindowV1:
    ...
    cutoff = datetime.combine(local.date(), session_policy.new_entry_cutoff, tzinfo=local.tzinfo)
    close = datetime.combine(local.date(), session_policy.regular_close, tzinfo=local.tzinfo)
    if local >= close:
