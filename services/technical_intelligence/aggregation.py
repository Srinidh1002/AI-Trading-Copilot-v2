...
def aggregate_technical_intelligence(*,multi_timeframe_snapshot,multi_timeframe_quality_result,timeframe_technical_evidence,policy=None,clock=None,technical_intelligence_result_id_factory=None):
 ...
 supplied=tuple(x.timeframe for x in timeframe_technical_evidence)
 if multi_timeframe_quality_result.multi_timeframe_snapshot_id!=multi_timeframe_snapshot.multi_timeframe_snapshot_id or supplied!=tuple(t for t in policy.required_timeframes if t in supplied) or tuple(multi_timeframe_snapshot.required_timeframes)!=policy.required_timeframes or any(t not in supplied for t in policy.mandatory_timeframes):raise ValueError("Aggregation linkage mismatch.")
 ...
 weights=dict(policy.timeframe_weights);weighted=sum(score*weights[e.timeframe] for score,e in zip(scores,timeframe_technical_evidence));missing=tuple(t for t in policy.optional_timeframes if t not in supplied);unavailable=missing+tuple(e.timeframe for e in timeframe_technical_evidence if e.blockers or e.valid_indicator_count==0);bull=tuple(e.timeframe for e,s in zip(timeframe_technical_evidence,scores) if e.timeframe not in unavailable and s>=.1);bear=tuple(e.timeframe for e,s in zip(timeframe_technical_evidence,scores) if e.timeframe not in unavailable and s<=-.1);neutral=tuple(t for t in policy.required_timeframes if t not in bull+bear+unavailable)
 mixed=bool(bull and bear);bias="MIXED" if mixed else "BULLISH" if weighted>=.15 else "BEARISH" if weighted<=-.15 else "NEUTRAL";final_status="READY_WITH_WARNINGS" if status=="READY_WITH_WARNINGS" or mixed or status=="MISALIGNED" or unavailable else "READY";blockers=tuple(multi_timeframe_quality_result.blockers);warnings=tuple(multi_timeframe_quality_result.warnings)
 if unavailable and not blocking:
  warnings=tuple(dict.fromkeys(warnings+tuple(f"technical_timeframe_unavailable_{t}" for t in unavailable)))
