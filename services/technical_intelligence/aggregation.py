...
def aggregate_technical_intelligence(*,multi_timeframe_snapshot,multi_timeframe_quality_result,timeframe_technical_evidence,policy=None,clock=None,technical_intelligence_result_id_factory=None):
 ...
 weighted=sum(score*weight for score,(_,weight) in zip(scores,policy.timeframe_weights));unavailable=tuple(e.timeframe for e in timeframe_technical_evidence if e.blockers or e.valid_indicator_count==0);bull=tuple(t for t,s in zip(policy.required_timeframes,scores) if t not in unavailable and s>=.1);bear=tuple(t for t,s in zip(policy.required_timeframes,scores) if t not in unavailable and s<=-.1);neutral=tuple(t for t in policy.required_timeframes if t not in bull+bear+unavailable)
