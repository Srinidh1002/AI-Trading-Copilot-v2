...
def evaluate_level_intelligence(series, policy=None) -> dict[str,object]:
 ...
 prior=c[-minimum:-1];last=c[-1];support=min(x.low_price for x in prior);resistance=max(x.high_price for x in prior);range_=max(resistance-support,1e-12);support_distance=(last.close_price-support)/range_;resistance_distance=(resistance-last.close_price)/range_;near_support=0.0<=support_distance<=.1;near_resistance=0.0<=resistance_distance<=.1;breakout=last.close_price>resistance;breakdown=last.close_price<support
