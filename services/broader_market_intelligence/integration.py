...
def build_broader_market_intelligence(*,primary_series:MarketCandleSeriesV1,related_series:MarketCandleSeriesV1,breadth_snapshot:MarketBreadthSnapshotV1|None=None,volatility_snapshot:VolatilitySnapshotV1|None=None,policy:BroaderMarketIntelligencePolicyV1=DEFAULT_BROADER_MARKET_INTELLIGENCE_POLICY,created_at:datetime,cross_market_evidence_id:str,breadth_evidence_id:str|None=None,volatility_context_id:str|None=None,result_id:str,cross_market_created_at:datetime|None=None,correlation_evaluator:Callable=evaluate_cross_market_correlation,breadth_evaluator:Callable=evaluate_market_breadth,volatility_evaluator:Callable=evaluate_volatility_context,aggregate_evaluator:Callable=evaluate_broader_market_intelligence):
 ...
 cross_market_created_at=created_at if cross_market_created_at is None else cross_market_created_at
 if not isinstance(cross_market_created_at,datetime) or cross_market_created_at.tzinfo is None or cross_market_created_at.utcoffset() is None:raise ValueError("cross_market_created_at must be timezone-aware")
 if cross_market_created_at>created_at:raise ValueError("cross_market_created_at cannot follow created_at")
 ...
 correlation=correlation_evaluator(primary_series=primary_series,related_series=related_series,policy=policy,created_at=cross_market_created_at,evidence_id=cross_market_evidence_id)
