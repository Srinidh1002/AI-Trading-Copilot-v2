...
def evaluate_cross_market_correlation(*,primary_series:MarketCandleSeriesV1,related_series:MarketCandleSeriesV1,policy:BroaderMarketIntelligencePolicyV1=DEFAULT_BROADER_MARKET_INTELLIGENCE_POLICY,created_at:datetime,evidence_id:str)->CrossMarketEvidenceV1:
 ...
 if policy.require_same_timeframe and primary_series.timeframe!=related_series.timeframe:return _blocked(primary_series,related_series,policy,created_at,evidence_id,"MISALIGNED","CROSS_MARKET_TIMEFRAME_MISALIGNED")
 pt,rt=_source_timestamp(primary_series),_source_timestamp(related_series)
 if max((pt-created_at).total_seconds(),(rt-created_at).total_seconds())>policy.future_timestamp_tolerance_seconds:return _blocked(primary_series,related_series,policy,created_at,evidence_id,"BLOCKED","CROSS_MARKET_SOURCE_TIMESTAMP_FUTURE")
 if max((created_at-pt).total_seconds(),(created_at-rt).total_seconds())>policy.maximum_cross_market_age_seconds:return _blocked(primary_series,related_series,policy,created_at,evidence_id,"STALE","CROSS_MARKET_SOURCE_EVIDENCE_STALE")
 if abs((pt-rt).total_seconds())>policy.maximum_cross_market_timestamp_skew_seconds:return _blocked(primary_series,related_series,policy,created_at,evidence_id,"MISALIGNED","CROSS_MARKET_SOURCE_TIMESTAMP_SKEW")
