"""Build an exact selected-market P6 bundle from retained Task 9 evidence.
...
def _effective_task9_risk_fraction(
    runtime_risk_fraction: object,
) -> float:
    value = _positive(runtime_risk_fraction, "risk_fraction")
    if value > 1.0:
        raise ValueError("risk_fraction")
    return min(value, _MAXIMUM_RISK_FRACTION)


def _task9_quantity_lot_limit(
    *,
    ...
    size = int(_positive(lot_size, "lot_size"))
    if size != lot_size:
        raise ValueError("lot_size")
    limit = min(_MAXIMUM_PLANNED_LOTS, maximum_quantity // size)
    if limit < 1:
        raise ValueError("selected contract exceeds runtime maximum_quantity")
    return limit


def classify_task9_expiry(expiry: date) -> str:
...
def derive_task9_strike_interval(
    *,
...
def derive_task9_moneyness_steps(
    *,
    strike: float,
    spot_price: float,
...
def _validate_selected_inputs(
    *,
    ...
    if market not in _SUPPORTED_MARKETS:
        raise ValueError("unsupported Task 9 market")
...
def _planning_policy(
    *,
    policy_id: str,
    maximum_risk_fraction: float = _MAXIMUM_RISK_FRACTION,
    maximum_lot_count: int = _MAXIMUM_PLANNED_LOTS,
) -> TradePlanningPolicyV1:
    return TradePlanningPolicyV1(
        policy_id=policy_id,
        policy_name="TASK9_SELECTED_MARKET_PAPER",
        policy_version="1",
        maximum_risk_fraction=maximum_risk_fraction,
...
def build_task9_selected_market_p6_bundle(
    *,
    bridge: SelectedMarketPlanningBridgeResultV1,
    cycle: PaperOrchestrationCycleInputV1,
    evaluation: LiveMarketCandidateEvaluationResultV1,
    ...
    evaluated_at: datetime,
    risk_fraction: float = _MAXIMUM_RISK_FRACTION,
    ...
    effective_risk_fraction = _effective_task9_risk_fraction(
        risk_fraction,
    )
    ...
    quantity_lot_limit = (
        ...
        else _task9_quantity_lot_limit(
    ...
    strike_interval = derive_task9_strike_interval(
        strikes=same_expiry_strikes,
    )
    moneyness_steps = derive_task9_moneyness_steps(
    ...
    expiry_category = classify_task9_expiry(
        selected_contract.expiry_date
    )
    ...
    policy_id = (
        f"task9-p6-policy:{cycle.cycle_id}"
    )
    policy = _planning_policy(
        policy_id=policy_id,
        maximum_risk_fraction=effective_risk_fraction,
        maximum_lot_count=quantity_lot_limit,
    )

    decision = FinalDecisionV1(
        snapshot_id=cycle.observation_id,
        decision_id=(
            f"task9-final-decision:{cycle.cycle_id}"
        ),
        symbol=symbol,
        exchange=exchange,
        ...
        risk=RiskSummary(
            ...
            capital=capital,
            risk_amount=(
                capital * effective_risk_fraction
            ),
        ),
        supporting_reasons=(
        ...
        engine_versions={
            "task9_candidate_evaluator": "1",
            "trade_opportunity_integrator": "1",
        },
        rule_versions={
            "task9_selected_market_p6": "1",
        },
        source_timestamps={
        ...
        trace_metadata={
            "pipeline": "task9_selected_market_p6",
    ...
    if not decision.validation_passed:
        raise ValueError(
            "Task 9 final decision failed validation: "
    ...
    trade_opportunity = build_canonical_trade_opportunity(
        ...
        opportunity_id_factory=lambda: (
            f"task9-opportunity:{cycle.cycle_id}"
        ),
    )
    ...
    market_opportunity = MarketOpportunityCandidateV1(
        candidate_id=(
            f"task9-market-opportunity:{cycle.cycle_id}"
        ),
        evaluated_at=now,
        ...
        metadata={
            "task9_parent_cycle_id": bridge.parent_cycle_id,
            "task9_selected_child_result_id": (
    ...
    trade_plan_input_id = (
        f"task9-trade-plan-input:{cycle.cycle_id}"
    )
    canonical = CanonicalTradePlanInputV1(
        ...
        metadata={
            "task9_observation_id": cycle.observation_id,
            "task9_ranking_id": ranking.ranking_id,
        },
    )
    ...
    eligibility = tuple(
        OptionContractEligibilityEvidenceV1(
            evidence_id=(
                f"task9-eligibility:{cycle.cycle_id}:"
            ...
            evidence_source="TASK9_RETAINED_OPTION_UNIVERSE",
    ...
    selection_id = (
        f"task9-option-selection:{cycle.cycle_id}"
    )
    selection_result_id = (
    ...
    target_evaluation_id = (
        f"task9-targets:{cycle.cycle_id}"
    )
    target_result_id = (
    ...
    placeholder_selection = (
        OptionContractSelectionResultV1(
            ...
            selected_reason_codes=("TASK9_RETAINED_RANKING",),
    ...
    capital_policy = CapitalQuantityPlanningPolicyV1(
        ...
        policy_source="TASK9_CERTIFIED",
    )
    cost_policy = CapitalQuantityTradingCostPolicyV1(
        cost_policy_id=(
            f"task9-cost-policy:{cycle.cycle_id}"
        ),
    ...
    planning_input_id = (
        f"task9-capital-input:{cycle.cycle_id}"
    )
    trade_plan_id = (
        f"task9-trade-plan:{cycle.cycle_id}"
    )
    ...
    if (
        ...
        raise ValueError(
            "Task 9 requires complete affordability "
    ...
    capital_input = CapitalQuantityPlanningInputV1(
        ...
        evaluated_at=now,
        input_source="TASK9_RETAINED_CERTIFIED_EVIDENCE",
        warnings=warnings,
    ...
    task9_cost_policy = CapitalQuantityTradingCostPolicyV1(
        cost_policy_id=(
            f"task9-local-cost-policy:{capital_input.planning_input_id}"
        ),
        calculation_mode="FIXED_ASSUMPTION_MODEL",
        brokerage_fixed_per_order=20.0,
        exchange_transaction_charge_fraction=0.0005,
        ...
        slippage_rate_fraction=0.005,
        estimated_order_count=5,
        policy_timestamp=capital_input.evaluated_at,
        policy_source="TASK9_LOCAL_FIXED_ASSUMPTION_MODEL",
        source_timestamps=dict(capital_input.source_timestamps),
        metadata={"cost_authority": "TASK9_LOCAL"},
    )

    inherited_cost_evidence = capital_input.trading_cost_evidence
    if inherited_cost_evidence is None:
        raise ValueError(
            "Task 9 selected-market P6 requires structural cost evidence"
        )

    task9_cost_evidence = calculate_task9_local_paper_cost_evidence(
        policy=task9_cost_policy,
        planning_input_id=capital_input.planning_input_id,
        trade_plan_id=capital_input.trade_plan_id,
        option_selection_result_id=(
            capital_input
            .option_contract_selection_result
            .selection_result_id
        ...
        planned_lot_count=(
            inherited_cost_evidence.planned_lot_count
        ),
        lot_size=inherited_cost_evidence.lot_size,
        estimated_premium_outlay=(
            inherited_cost_evidence.estimated_premium_outlay
        ),
        evidence_timestamp=capital_input.evaluated_at,
        evidence_id=(
            f"task9-local-cost-evidence:"
            f"{capital_input.planning_input_id}"
        ),
        warnings=capital_input.warnings,
        source_timestamps=dict(capital_input.source_timestamps),
        metadata={
            "replaces_cost_policy_id": (
                capital_input
                .trading_cost_policy
                .cost_policy_id
            ),
            "authority": "TASK9_SELECTED_MARKET_P6",
        },
    )
