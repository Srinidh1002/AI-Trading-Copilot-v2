...
class FyersRawDataSocketV2(Protocol):
    ...
    def subscribe(self, *, symbols: Sequence[str], data_type: str) -> object: ...
    def unsubscribe(self, *, symbols: Sequence[str], data_type: str) -> object: ...
...
def _stream_access_token(*, access_token: str, client_id: str | None) -> str:
    token = str(access_token).strip()
    if not token:
        raise FyersStreamingConfigurationError("access_token is required")
    if ":" in token:
        return token
    app_id = str(client_id).strip() if client_id is not None else ""
    if not app_id:
...
def _install_fyers_ipv4_filter():
    ...
    def filtered(host, port, family=0, type=0, proto=0, flags=0):
        if isinstance(host, str) and (
            host.endswith("fyers.in") or host.endswith("fyersapi")
        ):
...
def _instrument_metadata(
    ...
    if not isinstance(instrument, Mapping):
        raise FyersStreamingSubscriptionError("instrument must be a mapping")
    symbol = instrument.get("provider_symbol")
    if not isinstance(symbol, str) or not symbol.strip():
        raise FyersStreamingSubscriptionError("provider_symbol is required")
    symbol = symbol.strip()
    return symbol, {
        "provider_token": instrument.get("provider_token"),
        "canonical_instrument_id": instrument.get("canonical_instrument_id"),
        "market_symbol": instrument.get("market_symbol"),
...
class FyersStreamingDataProviderV2:
    ...
    def __init__(
        ...
        *,
        ...
        if not log_path:
            raise FyersStreamingConfigurationError("log_path is required")
        ...
        self._latest_by_symbol: dict[str, dict[str, object]] = {}
    ...
    def _ensure_socket_locked(self) -> tuple[FyersRawDataSocketV2, bool]:
        if self._closed:
            raise FyersStreamingError("FYERS_STREAMING_PROVIDER_CLOSED")
        if self._socket is not None:
            return self._socket, False
        if self._ipv4_only and self._restore_dns is None:
            self._restore_dns = _install_fyers_ipv4_filter()
        try:
            ...
            raise FyersStreamingError("FYERS_DATA_SOCKET_CONSTRUCTION_FAILED") from exc
        self._access_token = ""
        return self._socket, True

    def _start_thread(self, raw_socket: FyersRawDataSocketV2) -> None:
        with self._lock:
    ...
    def _run_socket(self, raw_socket: FyersRawDataSocketV2) -> None:
        try:
            raw_socket.connect()
        except Exception as exc:
            with self._lock:
                ...
                self._last_error = f"connect:{type(exc).__name__}"
                self._error_count += 1

    def _subscribe_physical(
        self, raw_socket: FyersRawDataSocketV2, symbols: set[str]
    ) -> None:
        if symbols:
            raw_socket.subscribe(
                symbols=tuple(sorted(symbols)), data_type=self.DATA_TYPE
            )
    ...
    def _on_connect(self) -> None:
        ...
        if raw_socket is None:
            return
        try:
            self._subscribe_physical(raw_socket, desired)
        except Exception as exc:
            with self._lock:
                self._connected = False
                self._connected_event.clear()
                self._last_error = f"resubscribe:{type(exc).__name__}"
                self._error_count += 1
            return
        with self._lock:
    ...
    def _on_error(self, message) -> None:
        with self._lock:
            ...
            self._connected_event.clear()
            self._last_error = (
                message[:160] if isinstance(message, str) else type(message).__name__
            )
            self._error_count += 1
    ...
    def _normalize_message(
        ...
        symbol = symbol.strip()
        ltp = _coerce_number(
            message.get("ltp", message.get("last_traded_price", message.get("lp")))
        )
        ...
        provider_dt = _provider_datetime(provider_timestamp)
        provider_token = message.get("fyToken") or message.get("fy_token")
        return {
            "provider": "FYERS",
            "provider_symbol": symbol,
            "provider_token": provider_token,
            "token": provider_token or symbol,
            "token_source": "PROVIDER_TOKEN" if provider_token else "PROVIDER_SYMBOL",
            "ltp": float(ltp),
            "ts": provider_dt or received_at,
            "provider_timestamp": provider_timestamp,
            "timestamp_source": "PROVIDER" if provider_dt is not None else "LOCAL_RECEIPT",
    ...
    def _on_message(self, message) -> None:
        ...
        for item in items:
            ...
            callbacks: list[tuple[Callable, dict[str, object]]] = []
            with self._lock:
                if self._closed:
                    return
                ...
                for subscription in self._subscriptions.values():
                    metadata = subscription["instruments"].get(symbol)
                    if metadata is not None:
                        callbacks.append((subscription["callback"], metadata))
            for callback, metadata in callbacks:
                record = dict(base)
                instrument_token = metadata.get("provider_token")
                if record.get("provider_token") is None and instrument_token is not None:
    ...
    def subscribe(
        ...
        for instrument in instruments:
            symbol, metadata = _instrument_metadata(instrument)
        ...
        with self._lock:
            ...
            prospective = old_desired | set(metadata_by_symbol)
            if len(prospective) > self.MAX_SYMBOLS:
                raise FyersStreamingSubscriptionError(
                    "FYERS_STREAM_SYMBOL_LIMIT_EXCEEDED"
                )
            self._counter += 1
            subscription_id = f"fyers-stream-{self._counter}"
            ...
            newly_desired = prospective - old_desired
        if created:
            self._start_thread(raw_socket)
            return subscription_id
        if connected and newly_desired:
            try:
                self._subscribe_physical(raw_socket, newly_desired)
            except Exception as exc:
                with self._lock:
                    self._subscriptions.pop(subscription_id, None)
                    self._last_error = f"subscribe:{type(exc).__name__}"
                    self._error_count += 1
                raise FyersStreamingError("FYERS_STREAM_SUBSCRIBE_FAILED") from exc
            with self._lock:
                self._physical_symbols.update(newly_desired)
        return subscription_id

    def unsubscribe(self, subscription_id: str) -> None:
        subscription_id = str(subscription_id).strip()
        if not subscription_id:
            raise FyersStreamingSubscriptionError("subscription_id is required")
        with self._lock:
            ...
            for key, item in self._subscriptions.items():
                if key != subscription_id:
                    remaining.update(item["symbols"])
            remove_physical = old_desired - remaining
            connected = self._connected
            raw_socket = self._socket
        if connected and raw_socket is not None and remove_physical:
            try:
                self._unsubscribe_physical(raw_socket, remove_physical)
            except Exception as exc:
                with self._lock:
                    self._last_error = f"unsubscribe:{type(exc).__name__}"
                    self._error_count += 1
                raise FyersStreamingError("FYERS_STREAM_UNSUBSCRIBE_FAILED") from exc
        with self._lock:
            self._subscriptions.pop(subscription_id, None)
            self._physical_symbols.difference_update(remove_physical)
    ...
    def get_latest(self, provider_symbol: str) -> Mapping[str, object] | None:
        symbol = str(provider_symbol).strip()
        with self._lock:
            record = self._latest_by_symbol.get(symbol)
            return dict(record) if record is not None else None

    def is_healthy(self, stale_seconds: float = 30.0) -> bool:
        if stale_seconds <= 0:
        ...
        with self._lock:
            if not self._connected or self._last_message_at is None:
                return False
            age = (now - self._last_message_at).total_seconds()
    ...
    def snapshot(self) -> Mapping[str, object]:
        with self._lock:
            return {
                ...
                "desired_symbol_count": len(self._desired_symbols_locked()),
                "physical_symbol_count": len(self._physical_symbols),
