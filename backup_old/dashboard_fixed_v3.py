# dashboard_fixed_v3.py - FINAL FIXED DASHBOARD
...
def get_market_data():
    ...
    try:
        ...
        if market_data:
            if 'NIFTY' in market_data:
                data['nifty_ltp'] = market_data['NIFTY'].get('ltp', data['nifty_ltp'])
            if 'SENSEX' in market_data:
                data['sensex_ltp'] = market_data['SENSEX'].get('ltp', data['sensex_ltp'])
            data['source'] = 'WebSocket'
            data['using_fallback'] = True
    except Exception as e:
        pass
    
    # Generate simulated option chains (always available)
    ...
    # Try Angel One as secondary (for real data)
    try:
    ...
    # Ensure chains are never None
    if data['nifty_chain'] is None:
        data['nifty_chain'] = generate_simulated_chain('NIFTY', data['nifty_ltp'])
    if data['sensex_chain'] is None:
        data['sensex_chain'] = generate_simulated_chain('SENSEX', data['sensex_ltp'])
...
def main():
    ...
    if data.get('authenticated') and not data.get('using_fallback'):
        st.markdown('<div class="status-connected">✅ Connected to Angel One (Real Data)</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="status-fallback">🔄 Using {data.get("source", "Fallback")} Data (Simulated)</div>', unsafe_allow_html=True)
    ...
    # Get ATM data (safe access)
    nifty_chain = data.get('nifty_chain') or {}
    nifty_atm = get_atm_strike(nifty_chain)
    sensex_chain = data.get('sensex_chain') or {}
    sensex_atm = get_atm_strike(sensex_chain)
    ...
    with col5:
        pcr = nifty_chain.get('total_pcr', 0.5) if nifty_chain else 0.5
    ...
    with tab2:
        chain = data.get('sensex_chain') or {}
        df = create_option_chain_table(chain)
        if not df.empty:
            st.dataframe(df, use_container_width=True, height=400)
        else:
    ...
    with col2:
        pcr = sensex_chain.get('total_pcr', 0.5) if sensex_chain else 0.5
        if pcr < 0.5:
            action, color, emoji = "BUY CALL", "#00ff00", "🟢"
        elif pcr > 1.0:
            action, color, emoji = "BUY PUT", "#ff4444", "🔴"
    ...
    with st.sidebar.expander("🔐 Connection Status"):
        if data.get('authenticated'):
            st.success("✅ Angel One API (Real Data)")
        else:
            st.warning(f"🔄 {data.get('source', 'Fallback')} Data")
    
    # Show positions if available
    with st.sidebar.expander("📊 Active Positions"):
        st.info("Check trading engine logs for positions")
